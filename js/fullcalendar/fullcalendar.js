...
function AgendaView(element, calendar, viewName) {
	...
	function updateOptions() {
		...
		minMinute = 480;//parseTime(opt('minTime'));
		maxMinute = 1140;//parseTime(opt('maxTime'));
		colFormat = opt('columnFormat');
	...
	function buildSkeleton() {
		...
		if (opt('allDaySlot')) {
			...
			$('.fc-agenda-allday').remove(); // DONE INTENTIONALLY
			//$('.fc-table-head').remove(); // DONE INTENTIONALLY
			$('.fc-agenda-divider-inner').remove(); // DONE INTENTIONALLY
